...
n=5 #+/-= dixaine de noeuds par longueur d'onde
...
geo1=([[0.3*R,0.2*R,0.3*R,-0.2*R,-0.3*R,-0.2*R,-0.3*R,0.2*R]],1)

geo2=([  [0.3*R, 0.2*R, 0.3*R, -0.2*R, 0.1*R, -0.2*R, 0.1*R, 0.2*R],
        [-0.3*R, 0.2*R, -0.3*R, -0.2*R, -0.1*R, -0.2*R, -0.1*R, 0.2*R]
      ],2)

geo3= ([],3)
dx=0.04*R
slab_length=10  #longueur de la plaque en dx
interval= 1 #intervalle  entre les plaques en dx
slab_width=0.5 #largeur de la plaque en dx
for i in range(-17,20,round(slab_length/2+interval)):
    x1=dx*i
    y1=dx*slab_width
    x2=dx*i
    y2=-dx*slab_width
    x3=dx*(i-slab_length/2)
    y3=-dx*slab_width
    x4=dx*(i-slab_length/2)
    y4=dx*slab_width
    geo3[0].append([x1,y1,x2,y2,x3,y3,x4,y4])
    
geo4= ([],4)
dx=0.04*R
half_slab_length=5  #longueur de la plaque en dx
interval= 3 #intervalle  entre les plaques en dx
slab_width=0.5 #largeur de la plaque en dx
for i in range(-17,25,half_slab_length+interval):
    x1=dx*i
    y1=dx*slab_width
    x2=dx*i
    y2=-dx*slab_width
    x3=dx*(i-half_slab_length)
    y3=-dx*slab_width
    x4=dx*(i-half_slab_length)
    y4=dx*slab_width
    geo4[0].append([x1,y1,x2,y2,x3,y3,x4,y4])
...
interval= 3 #intervalle  entre les plaques en dx
slab_width=0.5 #largeur de la plaque en dx
for i in range(-9,23,half_slab_length+interval):
    x1=dx*i
    y1=dx*slab_width
    x2=dx*i
    y2=-dx*slab_width
    ...
    x4=dx*(i-half_slab_length)
    y4=dx*slab_width
    geo5[0].append([x1,y1,x2,y2,x3,y3,x4,y4])
geo5[0].append([-17*dx,dx*slab_width,-17*dx,-dx*slab_width,-24.9*dx,-dx*slab_width,-24.9*dx,dx*slab_width])
geo5[0].append([24.9*dx,dx*slab_width,24.9*dx,-dx*slab_width,18*dx,-dx*slab_width,18*dx,dx*slab_width])

geo=geo5

try:
    maillage=msh.GetData(R,nm,nd,lambda0,n,geo[1])
    print("mesh found")
    
except:
    print("creating mesh")
    msh.CreateMesh(R,nm,nd,lambda0,n,geo)
    maillage=msh.GetData(R,nm,nd,lambda0,n,geo[1])
    print("mesh created")
...
R0=(1/4)*np.array([[2,1],[1,2]]) ### a revoir !!!!!!!!!!!!!!!!
